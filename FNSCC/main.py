...
import numpy as np
#from utils.test import get_kmeans_centers


def run(args):
    args.resPath, args.tensorboard = setup_path(args)
    set_global_random_seed(args.seed)  #Set random seed

    train_loader = dataloader.explict_augmentation_loader(args) if args.augtype == "explicit" else dataloader.virtual_augmentation_loader(args) # Load data

    # model
    torch.cuda.set_device(args.gpuid[0])
    bert, tokenizer = get_bert(args) #Load the model and tokenizer

    #Cluster the sentence vectors obtained by bert to get the cluster center
...
def get_args(argv):
    ...
    parser.add_argument('--use_pretrain', type=str, default='SBERT', choices=["SBERT", "SBERT", "PAIRSUPCON"])

    parser.add_argument('--datapath', type=str, default='./AugData/Dataset/aug_20_bert+roberta/')
    parser.add_argument('--dataname', type=str, default='Tweet', help="")
    ...
    parser.add_argument('--neighbor', type=float, default=20, help="")
